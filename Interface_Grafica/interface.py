# =================================================================
# == EGG0-1 - INTERFACE DE CONTROLE COM HANDSHAKE ROBUSTO       ==
# =================================================================
...
def read_serial():
    # Loop de leitura apenas para debug no console (as respostas oficiais sao lidas no send_commands)
    while ser and ser.is_open:
        try: 
            if ser.in_waiting > 0:
                pass 
                # Deixamos a leitura para o send_commands quando estiver enviando,
                # ou podemos ler aqui para logs se não estiver em envio.
                # Para simplificar e evitar conflito de leitura, 
                # deixamos o _wait_for_ack consumir os dados.
        except: pass
        time.sleep(0.1)

def send_commands():
    """ENVIA LOTE DE COMANDOS COM VERIFICAÇÃO DE RESPOSTA (HANDSHAKE)"""
    cmds = command_listbox.get(0, tk.END)
    ...
    def _run_batch():
        status_label.config(text="Iniciando Handshake...")
        try:
            ser.reset_input_buffer() 
            
            # 1. LIMPAR
            print(">> Enviando: LIMPARFILA")
            ser.write(b"LIMPARFILA\\")
            ...
            print(">> Enviando: EXECUTAR")
            ser.write(b"EXECUTAR\\")
...
init_db()
root = tk.Tk()
root.title("EGG0-1 Control (Safe Batch)")
